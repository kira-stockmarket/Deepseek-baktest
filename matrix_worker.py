...
def calculate_technicals(df: pd.DataFrame, lookback: int) -> pd.DataFrame:
    ...
    rolling_max = df["INDEX"].rolling(lookback).max()
    rolling_min = df["INDEX"].rolling(lookback).min()
...
def fetch_and_prep_data(sector_name: str, lookback: int):
    ...
    print(f"Fetching data for {sector_name} ({ticker})...")
    
    idx_raw = yf.download(ticker, period="max", auto_adjust=True, progress=False)
    ...
    idx_s = idx_raw["Close"].rename("INDEX")
    vix_s = vix_raw["Close"].rename("VIX")
    df = pd.concat([idx_s, vix_s], axis=1).dropna()

    df = calculate_technicals(df, lookback)

    fwd_ret = df["INDEX"].shift(-5) / df["INDEX"] - 1
    # 0: Bearish (< -0.8%), 1: Choppy (-0.8% to +0.8%), 2: Bullish (> +0.8%)
    df["Target"] = np.where(fwd_ret < -0.008, 0, np.where(fwd_ret > 0.008, 2, 1))

    df = df.dropna()
    features = [c for c in df.columns if c not in ["INDEX", "VIX", "Target"]]
    return df, features
...
def main():
    ...
    print(f"[{args.sector}] Training {args.model} ({args.lookback}D)...")
    ...
    if args.model == "xgboost":
        ...
        clf = RandomForestClassifier(
            n_estimators=150, max_depth=6, class_weight="balanced_subsample", random_state=42
        )
        clf.fit(X_train, y_train)
        preds = clf.predict_proba(X_test)
    ...
    # Save artifacts with sector prefix
